...
public:
    ...
    Object remove_after(T_DLListNode<Object> &p)
    {
            ...
            {
                std::cout << "The list is empty or Nothing after\n";
            }
    ...
    Object remove_before(T_DLListNode<Object> &p)
    {
            ...
            {
                std::cout << "The list is empty or Nothing before\n";
            }
    ...
    int count = 0;
    int count_Nodes(T_DLListNode<Object> *currNode)
    {
        if(currNode != &trailer)
        {
            count++;
            count_Nodes(currNode->next);
        }
        else
        {
            int send = count;
...
/* Takes in 2 lists and returns a list which has them both merged */
template <typename Object>
DLList<Object> mergeList(DLList<Object> List1, DLList<Object> List2)
{
	DLList<Object> merge;	
	T_DLListNode<Object> *currNode = merge.first_node()->next;
	T_DLListNode<Object> *L1 = List1.first_node()->next;
	T_DLListNode<Object> *L2 = List2.first_node()->next;

	while(L1 != List1.after_last_node() && L2 != List2.after_last_node())
	{
		if(L1->obj < L2->obj)
		{
			currNode->next = L1;
    ...
    //If one of the lists runs out then the remaining list will be appended to the back of the list.
	if(L2 == List2.after_last_node())
	{
		currNode->next = L1;
	}
	if(L1 == List1.after_last_node())
	{
		currNode->next = L2;
	}
